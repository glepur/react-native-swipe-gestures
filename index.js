...
class GestureRecognizer extends Component {
  constructor(props, context) {
    ...
    let handleMove;
    if (
      this.props.onSwipeDown === undefined &&
      this.props.onSwipeUp === undefined
    ) {
      handleMove = this._handleMove.bind(this);
    } else {
      handleMove = shouldSetResponder;
    }
    ...
    this._panResponder = PanResponder.create({
      ...
      onMoveShouldSetPanResponder: handleMove,
  ...
  _handleMove(evt, gestureState) {
    if (Math.abs(gestureState.dy) < 4) {
      return this._handleShouldSetPanResponder(evt, gestureState);
    }
    return false;
  }
